    ...
    <blockquote class="layui-elem-quote layui-form">
      <div class="layui-input-inline" style="width:100px;">
        <select name="searchfield">
          ...
          <option value="role_name" selected="selected">角色</option>
        </select>
      </div>
...
layui.use(['table'], function () {
  ...
  $("#addBtn").click(function () {
    layer.open({
      ...
      area: ['350px', '400px'],
      title: '角色管理 - 新增',
      content: '<?php echo $this->baseUrl();?>/role/add',
  ...
  table.on('tool(data-list-event)', function (obj) {
    ...
    if (curEvent === 'relation') { //关联
      ...
      layer.open({
        ...
        area: ['350px', '400px'],
        title: '角色管理 - 编辑',
        content: '<?php echo $this->baseUrl();?>/role/edit?id=' + data.id,
        success : function (layero, index) {
          setTimeout(function () {
            layui.layer.tips('点击此处返回', '.layui-layer-setwin .layui-layer-close', {
              tips: 3
      ...
      if (data.id == 1) {
        layer.alert('您选择的是系统默认角色，不可删除！', {icon:0,anim:5,offset:'100px'});
        return false;
      }
      layer.confirm('确认要删除 ' + data.role_name + ' 吗？（删除后无法恢复）', function (index) {
        layer.close(index);
        $.ajax({
          type: 'get',
          url: '<?php echo $this->baseUrl();?>/role/del?id=' + data.id,
          dataType: 'json',
